...
public class MsgsGet {
	public static JSONArray callMsgsComments(String hostname, String token,
			String proId, String contentId)
			throws Exception {
		...
		HttpGet httget = new HttpGet();
			...
			httget.setURI(new URI(hostname+"projects/"+proId+"/messages/"+contentId+"/comments"));
		
		// httget.setURI(new
		// URI("http://192.168.1.78:8010/restapi/articles/list/"));
		System.out.println(hostname+"projects/"+proId+"/messages/"+contentId+"/comments");
	...
	// Creating notes
		public static JSONObject createMsg(JSONObject json, String token,
			...
			ClientConnectionManager ccm = new ThreadSafeClientConnManager(params,
					registry);
			HttpClient client = new DefaultHttpClient(ccm, params);
			String hostnamee = hostname + "projects/" + proId + "/messages";
