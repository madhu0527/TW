...
public class FilesGet {
	public static JSONObject callFilesList(String hostname, String token,String proId, String nextCall)
			throws Exception {
		...
		if(nextCall==""){
			httget.setURI(new URI(hostname+"projects/"+proId+"/files"));
		}else{
			httget.setURI(new URI(nextCall));
		}
		System.out.println(hostname+"projects/"+proId+"/files" + " _________________________________ ");
	...
	public static JSONArray callFilesComments(String hostname, String token,String proId,String fileId)
			throws Exception {
		SSLSocketFactory sslFactory = new SimpleSSLSocketFactory(null);
		sslFactory
				.setHostnameVerifier(SSLSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER);
		...
		HttpGet httget = new HttpGet();
			...
			httget.setURI(new URI(hostname+"projects/"+proId+"/files/"+fileId+"/comments"));
		
		// httget.setURI(new
		// URI("http://192.168.1.78:8010/restapi/articles/list/"));
		System.out.println(hostname+"projects/"+proId+"/files/"+fileId+"/comments");
		httget.setHeader("Accept", "*/*");
		httget.setHeader("Accept-Encoding", "identity");
		httget.addHeader("Connection", "keep-alive");
		httget.setHeader("User-Agent", "nnst");
	...
	public static JSONObject callFilesDetail(String hostname, String token,String proId, String fileId)
			throws Exception {
		SSLSocketFactory sslFactory = new SimpleSSLSocketFactory(null);
		sslFactory
				.setHostnameVerifier(SSLSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER);
