...
public class EventsGet {
	public static JSONArray callEventsList(String hostname, String token,
			String proId, String startDate, String endDate) throws Exception {
		...
		HttpGet httget = new HttpGet();
			httget.setURI(new URI(hostname + "projects/" + proId + "/event-occurrences?end="+endDate+"&start="+startDate));
		System.out.println(hostname + "projects/" + proId + "/event-occurrences?end="+endDate+"&start="+startDate);
	...
	public static JSONArray callMileStonessList(String hostname, String token,
			String proId, String startDate, String endDate) throws Exception {
		SSLSocketFactory sslFactory = new SimpleSSLSocketFactory(null);
		sslFactory
				.setHostnameVerifier(SSLSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER);
		...
		HttpGet httget = new HttpGet();
			httget.setURI(new URI(hostname + "projects/" + proId + "/milestones?end="+endDate+"&start="+startDate));
		System.out.println(hostname + "projects/" + proId + "/milestones?end="+endDate+"&start="+startDate);
	...
	public static JSONObject callEventsDetail(String hostname, String token,
			String eventurl) throws Exception {
		SSLSocketFactory sslFactory = new SimpleSSLSocketFactory(null);
		sslFactory
				.setHostnameVerifier(SSLSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER);
		HttpParams params = new BasicHttpParams();
		...
		HttpGet httget = new HttpGet();
		httget.setURI(new URI(hostname + eventurl.substring(1)));
		System.out.println(hostname + eventurl);
		httget.setHeader("Accept", "*/*");
		httget.setHeader("Accept-Encoding", "identity");
		httget.addHeader("Connection", "keep-alive");
		httget.setHeader("User-Agent", "nnst");
	...
	public static JSONArray CalenderEventsList(String hostname, String token,
		...
		HttpGet httget = new HttpGet();
			httget.setURI(new URI(hostname + "projects/calender?start="+startDate+"&end="+endDate));
		System.out.println(hostname + "projects/calender?start="+startDate+"&end="+endDate);
	...
	// Creating notes
		public static JSONObject createEvents(JSONObject json, String token,
			...
			ClientConnectionManager ccm = new ThreadSafeClientConnManager(params,
					registry);
			HttpClient client = new DefaultHttpClient(ccm, params);
			String hostnamee = hostname + "projects/" + proId + "/events";
